/*
  삼항연산자 : 조건 ? 참 : 거짓
  제어문
  	  IF: 만약에~~[경우의 수 판단]
  	  1.형태
  	  	1.	if( 조건 ) [참]실행문
  	  
  	  	2. 	if(조건)	{[true]실행문1; 실행문2; 실행문3;}
  	  		*실행문 2개 이상일떄 {}처리 하자
  	  	3. if(조건)	[[true]실행문; ]
  	  		else{ [false] 실행문; } 그기 아니라면~	
  	  		
  	  	4.if(조건) {[teue]실행문;}
  	  		else if(조건2) {[true2]실행문;}
  	  		else if(조건3) {[true3]실행문;}
  	  		else if(조건3) {[true4]실행문;}		
  	  		else {[false]실행문; }	
  	  				
  	  		; : 세미콜론[명령어단위의 마침표]	/ : 콜론
	실행문 (한줄일땐 생략가능)
	실행문; 실행문;
	if() [참]실행문	[if()뒤에 ; 하지말기]
  	  	
  	  	
  	  	5. 중첩
  	  		if(조건1){
					
					if(조건2){
						if(조건3){
							
						}
						
					}
				}
  	  	
  	  	
  	  	
 */
//1. if 형태1
if(10>3)   console.log('[참] 10이 더 크다')
if(10>20)  console.log( '[참]10이 더 크다')
if(10>20); console.log( '[참]10이 더 크다')	//if절의 ; 끝맞침하였으니 참이 아니어도 console가 실행됨 즉 [x] 쓰지말것 

//2. if 형태2
if(10>3) console.log('참1'); console.log('10이 더 크다');	//[x]
if(10>20)  console.log('참2'); console.log('10이 더 크다'); //[x]
if(10>20) {console.log('참3'); console.log('10이 더 크다'); }//[o] 맞는형식이나 사실이 아니라 뜨지 않을거임

//3. if형태3
if(10>3){console.log('[참1] 10이 더 크다');}
else{ console.log ('[거짓1] 10이 더 작다');}
		//vs 삼황연산자 두개중 편한거로 사용 단 밑에거는 코드가 길어지면 힘듬
10>3 ? console.log ('[참2]') : console.log('[거짓2]')

//4.if 형태4
if(10>=20){console.log('[참1] 10이 20보다 이상.')}				//만약에 10>= 20 이상이면
else if (10 >= 15) {console.log('[참2] 10이 15보다 이상.')}		//아니면서 만약에 10>=15 이상이면
else if (10>=15) {console.log('[참3] 10이 10보다 이상.')}		//아니면서 만약에 10>=10 이상이면 
else {console.log('[거짓] 10이 10미만이다.')}					//그외
			//vs 삼황연산자
10 >= 20 ? console.log('[참1] 10이 20보다 이상.') :
10 >= 15 ? console.log('[참2] 10이 15보다 이상.') :
10 >= 10 ? console.log('[참3] 10이 10보다 이상.') :
console.log('[거짓] 10이 10미만이다.') 

/*
	1. prompt로 정수 1개 입력받아 90이상이면 '합격' 아니면 '탈락' console 출력
	2. prompt로 성별 입력받아
		'M'또는'm'또는 '남'또는'남자'일경우 '남자 이군요' console출력
		'W'또는'w'또는 '여'또는'여자'일경우 '여자 이군요' console출력
		그외 '남자 : m,M,남,남자 여자:w,W,여,여자로 입력해주세요' 라고 console출력
		
	3.	prompt로 점수1개 입력받아
		90점 이상이면 'A등급' 80점 이상이면 'B등급' 70점 이상이면 'C등급' 그외 '탈락'
		
*/

//1.
/*let 정수=Number(prompt('정수1'))
if( 정수>=90 ){ console.log("합격");}
else{console.log("탈락")}
// =
정수>=90 ? console.log("합격") : console.log("탈락")

*/

//2.
let 성별 = prompt('성별')
if( 성별=='M'||성별=='m'||성별=='남'||성별=='남자' ){ 
	console.log('남자이군요')
}else if ( 성별=='W'||성별=='w'||성별=='여'||성별=='여자') {
	console.log('여자이군요')
}else{ console.log('남자이면 m,M,남,남자 형식/ 여자이면 w,W,여,여자 형식으로 입력해주세요;')}

//3.
let점수=Number(prompt('점수입력'))

if(점수 >=90 ) {
	console.log('A등급');
} else if (점수 >= 80){ 
	console.log('B등급'); 
} else if(점수 >= 70){ 
	console.log('C등급'); 
} else { 
	console.log('탈락');
}









