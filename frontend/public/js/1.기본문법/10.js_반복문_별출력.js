...
/*문제6*/
output = ''      //초기화
let line6 = Number( prompt('문제6 줄수 ') )   //선언
for(let i=0; i<5; i++){            //i는 0부터 입력받은 줄까지 1씩 증가함
   for(let b=0; b<4-i; b++){      //b는 0부터 (입력받은 줄 수-현재줄수)  까지 1씩증가
      output += ' '
   }
...
for(let i=0; i<5; i++){      //i는 0부터 입력받은 줄까지 1씩 증가함      
   for(let b=0; b<i; b++){   //공백은 현재줄수  까지 1씩증가
      output += ' '
    }

   for( let s = 0; s<2*(5-i)-1; s++ ){   //별은 0부터 2*(입력받은줄수-현재줄수) 까지 1씩증가
      output += '*'   
   }   
   output += '\n'      //줄바꿔 
}
