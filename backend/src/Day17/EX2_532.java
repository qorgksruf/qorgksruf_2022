...
public class EX2_532 {
	public static void main(String[] args) {
		
		//1.Data, simpleDataFormat
			//date 함수는 주로 date객체명  = new date
		Date now = new Date();	//현재 시스템 날짜
		...
		System.out.println("yyyy.MM.dd a HH:mm:ss");
		
		dateformat = new SimpleDateFormat("오늘은 E 요일");
		System.out.println(dateformat.format(date));
		
		
		dateformat = new SimpleDateFormat("[연 기준]오늘은 D번째 날");
		System.out.println(dateformat.format(date));
		
		dateformat = new SimpleDateFormat("[월 기준]오늘은 d번째 날");
		System.out.println(dateformat.format(date));
